...
try {
  ...
  addCheck('Legacy spacing values', '❌ FAIL', 'Could not read tailwind.config.ts');
}
 ...
 */
try {
  const microInteractions = fs.readFileSync('src/styles/micro-interactions.css', 'utf-8');
  
  // Look for hardcoded easing functions that should use CSS variables
  const hardcodedEasingRegex = /animation:\s*[^;]*\s+(ease-in|ease-out|ease-in-out)(?!\s*var\()/g;
  ...
  addCheck('Hardcoded easing functions', '❌ FAIL', 'Could not read micro-interactions.css');
}
...
try {
  ...
  if (strictMode === true) {
    ...
    addCheck('TypeScript strict mode', '⚠️ WARNING', 'Strict mode is not enabled');
  }
} catch (error) {
...
try {
  ...
  const missingRules = requiredRules.filter(rule => !eslintConfig.includes(rule));
  ...
  addCheck('ESLint configuration', '❌ FAIL', 'Could not read eslint.config.js');
}
...
try {
  ...
  addCheck('Documentation completeness', '❌ FAIL', 'Error checking documentation');
}
...
try {
  ...
  const missingExports = expectedExports.filter(exportName => !uiIndex.includes(exportName));
