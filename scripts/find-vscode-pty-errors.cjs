...
const patterns = [
  /Could not find pty/i,
  /reconnection grace/i,
  /Could not find pty \d+/i,
];
...
function main() {
  ...
  if (!dirs.length) {
    console.log(
      "No VS Code log directories found automatically. You can pass a directory as an argument.",
    );
    console.log("Examples:");
    console.log(
      "  npm run diagnose:terminal -- C:\\Users\\you\\AppData\\Roaming\\Code\\logs",
    );
  }

  const searchDirs = process.argv.slice(2).length
