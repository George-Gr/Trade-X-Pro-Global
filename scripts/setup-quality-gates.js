...
const colorPatterns = [
  ...
  {
    pattern:
      /(backgroundColor|color|borderColor):\s*['"]?rgb[a]?\(\s*\d+\s*,\s*\d+\s*,\s*\d+/gi,
    name: "RGB/RGBA color",
    type: "error",
    autoFix: (match) =>
      match.replace(/rgb[a]?\([^)]+\)/, "hsl(var(--primary))"),
...
const typographyPatterns = [
  {
    ...
    validate: (value) => {
      const allowedSizes = [
        10, 11, 12, 14, 16, 18, 20, 24, 30, 36, 48, 56, 64, 72,
      ];
      return allowedSizes.includes(parseFloat(value));
    },
    suggestion:
      "Use approved text sizes: 10, 12, 14, 16, 18, 20, 24, 30, 36, 48, 56, 64, 72px",
  },
  {
  ...
  // Tailwind arbitrary font sizes
  {
    ...
    suggestion:
      "Use standard text classes: text-xs, text-sm, text-base, text-lg, text-xl, text-2xl",
  },
];
...
const spacingPatterns = [
  {
    ...
    suggestion:
      "Use 4px grid: 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 48, 56, 64, 72, 80, 88, 96, 128px",
  },
  {
  ...
  },
  {
    ...
    suggestion:
      "Use 4px grid: 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 48, 56, 64px",
  },
...
const borderRadiusPatterns = [
  {
    ...
    suggestion:
      "Use approved radii: 0, 2, 4, 6, 8, 12, 16, 24px (or rounded for full radius)",
  },
  {
    ...
    suggestion:
      "Use standard radius: rounded, rounded-sm, rounded-md, rounded-lg, rounded-xl, rounded-2xl",
  },
];
...
const cssVariablePatterns = [
  {
    ...
    type: "error",
    autoFix: (match) =>
      match.replace(/#([0-9a-fA-F]{3}){1,2}/, "hsl(var(--primary))"),
  ...
  const reportData = {
    ...
    summary: {
      ...
      complianceRate: Math.max(
        0,
        100 - ((violationCount + warningCount) / 100) * 10,
      ),
    },
    violations: REPORTS.violations,
    suggestions: REPORTS.suggestions,
    recommendations: {
      immediate: REPORTS.violations
...
function validateFile(filePath) {
  try {
    ...
    colorPatterns.forEach(
      ({ pattern, name, type: issueType, autoFix, suggestion, validate }) => {
        let match;
        pattern.lastIndex = 0;
    ...
    typographyPatterns.forEach(
      ({ pattern, name, type: issueType, validate, suggestion }) => {
        let match;
        pattern.lastIndex = 0;
        while ((match = pattern.exec(content)) !== null) {
          const lineNum = getLineNumber(content, match.index);
    ...
    cssVariablePatterns.forEach(
      ({ pattern, name, type: issueType, autoFix, suggestion }) => {
        let match;
        pattern.lastIndex = 0;
        while ((match = pattern.exec(content)) !== null) {
          const lineNum = getLineNumber(content, match.index);
    ...
    accessibilityPatterns.forEach(
      ({ pattern, name, type: issueType, suggestion }) => {
        let match;
        pattern.lastIndex = 0;
        while ((match = pattern.exec(content)) !== null) {
          const lineNum = getLineNumber(content, match.index);
...
async function main() {
  ...
  if (baselineFile && fs.existsSync(baselineFile)) {
    try {
      ...
      console.log(
        `   Previous violations: ${baseline.summary?.violations || 0}`,
      );
  ...
  console.log(
    `Found ${tsxFiles.length} TypeScript files and ${cssFiles.length} CSS files\n`,
  );
  ...
  if (baseline) {
    ...
    console.log(
      `   Violations: ${violationCount} (${violationDiff >= 0 ? "+" : ""}${violationDiff})`,
    );
    console.log(
      `   Warnings: ${warningCount} (${warningDiff >= 0 ? "+" : ""}${warningDiff})`,
    );
  ...
  if (baseline) {
    ...
    console.log(
      `   Compliance Change: ${complianceDiff >= 0 ? "+" : ""}${complianceDiff.toFixed(1)}%`,
    );
  }
  ...
  if (violationCount > FAILURE_THRESHOLD) {
    console.error(
      `❌ Design system validation failed with ${violationCount} violation(s)`,
    );
    ...
    if (isCI) {
      console.error(`\n🔧 CI/CD Integration:`);
      console.error(
        `   - Review violations in design-system-violations-report.json`,
      );
  ...
  if (warningCount > WARNING_THRESHOLD) {
    console.warn(
      `⚠️  ${warningCount} warning(s) found (threshold: ${WARNING_THRESHOLD})`,
    );
  ...
  if (violationCount === 0 && warningCount === 0) {
    ...
    reportSuggestion(
      "Excellence",
      "Outstanding! Consider setting stricter baselines for continuous improvement.",
      "low",
    );
  } else if (violationCount === 0) {
    console.log(
      "✅ No violations! Great job maintaining design system standards.",
    );
    if (warningCount > 0) {
      reportSuggestion(
        "Improvement",
        `Consider addressing ${warningCount} warning(s) to reach 100% compliance.`,
        "low",
      );
    }
  } else {
    console.log(
      `✅ Violations within acceptable range (${violationCount}/${FAILURE_THRESHOLD})`,
    );
  }
  ...
  if (duration > 5000) {
    reportSuggestion(
      "Performance",
      "Validation is slow. Consider optimizing file patterns or running validation only on changed files.",
      "medium",
    );
  }
  ...
  if (autoFixableCount > 0) {
    ...
    console.log(
      `   Run with --fix flag for automatic fixes (when implemented)`,
    );
  }

  console.log("\n📋 Next Steps:");
  console.log(
    "   - Review detailed report: design-system-violations-report.json",
  );
  console.log("   - Address high-priority violations");
  console.log(
    "   - Run baseline comparison: --baseline design-system-baseline.json",
  );
