  ...
  const fetchKycStatus = useCallback(async () => {
    ...
    try {
      const { data: profile, error } = await supabase
        .from('profiles')
        .select('kyc_status, kyc_rejected_at, kyc_approved_at, kyc_rejection_reason')
        .eq('id', user.id)
        .single();
      ...
      if (isRejected && (profile as unknown as Record<string, unknown>).kyc_rejected_at) {
        ...
        if (now >= resubmitDate) {
          ...
          const daysRemaining = Math.ceil((resubmitDate.getTime() - now.getTime()) / (24 * 60 * 60 * 1000));
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          const { kyc_status, kyc_rejected_at, kyc_approved_at, kyc_rejection_reason } =
            (payload.new as Record<string, unknown>) || {};

          if (kyc_status) {
            const status = kyc_status as KycTradingState['kycStatus'];
            const isApproved = status === 'approved';
            const isRejected = status === 'rejected' || status === 'requires_resubmit';
