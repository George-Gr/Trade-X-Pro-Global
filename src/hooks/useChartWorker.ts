...
export const useChartWorker = <T>() => {
  ...
  const [responses, setResponses] = useState<Map<string, WorkerResponse>>(new Map());
  ...
  const sendRequest = useCallback(<D>(type: string, data: D): Promise<T> => {
  ...
  const calculateTrend = useCallback((values: number[]): Promise<unknown> => {
    return sendRequest('calculateTrend', { values });
  }, [sendRequest]);

  const normalizeData = useCallback((data: { value: number }[]): Promise<unknown> => {
    return sendRequest('normalizeData', data);
  }, [sendRequest]);

  const generateSparkline = useCallback((values: number[], labels?: string[]): Promise<unknown> => {
    return sendRequest('generateSparkline', { values, labels });
  }, [sendRequest]);

  const optimizeData = useCallback((data: unknown[], maxPoints?: number): Promise<unknown> => {
    return sendRequest('optimizeData', { data, maxPoints });
  }, [sendRequest]);

  const getLoadingState = useCallback((type: string): boolean => {
