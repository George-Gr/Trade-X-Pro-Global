...
export function useSubscription(options: UseSubscriptionOptions) {
  const { table, event = '*', filter, onData, enabled = true, channelId } = options;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(id)
      .on('postgres_changes' as never, { event, schema: 'public', table, ...(filter ? { filter } : {}) } as never, onData as never)
      .subscribe();
