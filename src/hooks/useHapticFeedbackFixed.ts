...
export interface HapticFeedbackConfig {
  enabled?: boolean;
  pattern?: 'light' | 'medium' | 'heavy' | 'success' | 'warning' | 'error' | 'impact';
  duration?: number;
}
...
export const useHapticFeedback = (defaultEnabled = true): HapticFeedbackReturn => {
  ...
  const trigger = useCallback((config: HapticFeedbackConfig = {}) => {
