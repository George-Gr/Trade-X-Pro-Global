...
export const usePerformanceMonitoring = () => {
  ...
  const trackListener = useCallback((type: string, add: boolean = true) => {
    ...
    listenersRef.current.set(type, add ? current + 1 : Math.max(0, current - 1));
  ...
  return {
    ...
    trackTimer,
    cleanupAll,
    getListenerCount: () => Array.from(listenersRef.current.values()).reduce((sum, count) => sum + count, 0),
    getTimerCount: () => Array.from(timersRef.current.values()).reduce((sum, count) => sum + count, 0),
  };
};
