...
describe('useSLTPMonitoring - Core Logic', () => {
  ...
  it('should trigger stop loss for buy position when price drops below threshold', () => {
    const triggered = shouldTriggerStopLoss('long', 1.0849, 1.0850);
  ...
  it('should trigger take profit for buy position when price rises above threshold', () => {
    const triggered = shouldTriggerTakeProfit('long', 1.0951, 1.0950);
    expect(triggered).toBe(true);
  });
  ...
  it('should trigger stop loss for sell position when price rises above threshold', () => {
    const triggered = shouldTriggerStopLoss('short', 1.0951, 1.0950);
    expect(triggered).toBe(true);
  });
  ...
  it('should trigger take profit for sell position when price drops below threshold', () => {
    const triggered = shouldTriggerTakeProfit('short', 1.0849, 1.0850);
    expect(triggered).toBe(true);
  });
  ...
  it('should not trigger when price is between stop loss and take profit', () => {
    const slTriggered = shouldTriggerStopLoss('long', 1.0890, 1.0850);
    const tpTriggered = shouldTriggerTakeProfit('long', 1.0890, 1.0950);
  ...
  it('should check stop loss before take profit', () => {
    ...
    const priceAtSL = 1.0850;
    const slTriggered = shouldTriggerStopLoss('long', priceAtSL, 1.0850);
  ...
  it('should evaluate multiple positions independently', () => {
    const positions = [
      { side: 'long' as const, price: 1.0849, sl: 1.0850, tp: 1.0950 },
      { side: 'short' as const, price: 1.0951, sl: 1.0950, tp: 1.0850 },
      { side: 'long' as const, price: 1.0900, sl: 1.0850, tp: 1.0950 },
    ];
  ...
  it('should trigger at exact threshold values', () => {
    expect(shouldTriggerStopLoss('long', 1.0850, 1.0850)).toBe(true);
    expect(shouldTriggerTakeProfit('long', 1.0950, 1.0950)).toBe(true);
    expect(shouldTriggerStopLoss('short', 1.0950, 1.0950)).toBe(true);
    expect(shouldTriggerTakeProfit('short', 1.0850, 1.0850)).toBe(true);
  });
});
