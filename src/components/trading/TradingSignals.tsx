    ...
    return [
      {
        ...
        timeframe: "15M",
        strength: random(hash) > 0.7 ? "strong" : random(hash) > 0.4 ? "moderate" : "weak",
      ...
      },
      {
        ...
        timeframe: "1H",
        strength: random(hash + 1) > 0.7 ? "strong" : random(hash + 1) > 0.4 ? "moderate" : "weak",
        entry: (basePrice * (1 + (random(hash + 1) - 0.5) * 0.015)).toFixed(4),
        target: (basePrice * (1 + (random(hash + 1) - 0.5) * 0.03)).toFixed(4),
        stopLoss: (basePrice * (1 + (random(hash + 1) - 0.5) * 0.007)).toFixed(4),
        time: "15 min ago"
      },
      {
        type: random(hash + 2) > 0.5 ? "buy" : "sell",
        timeframe: "4H",
        strength: random(hash + 2) > 0.7 ? "strong" : random(hash + 2) > 0.4 ? "moderate" : "weak",
        entry: (basePrice * (1 + (random(hash + 2) - 0.5) * 0.02)).toFixed(4),
        target: (basePrice * (1 + (random(hash + 2) - 0.5) * 0.04)).toFixed(4),
        stopLoss: (basePrice * (1 + (random(hash + 2) - 0.5) * 0.01)).toFixed(4),
        time: "1 hour ago"
      }
    ];
  }, [symbol]);
        ...
        {signals.map((signal, index) => (
          <div key={index} className="p-3 rounded-lg border border-border space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Badge variant={getStrengthColor(signal.strength)} className="text-xs">
                {signal.strength}
              </Badge>
            </div>
