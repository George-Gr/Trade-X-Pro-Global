...
it('should render positions table with all positions', () => {
  ...
  it('should filter positions by side', async () => {
    render(<EnhancedPositionsTable />);
    const buyButton = screen.getAllByRole('button').find(btn => btn.textContent?.trim() === 'Buy');
...
describe('Integration Tests', () => {
  ...
  it('should handle filter changes efficiently', async () => {
    render(<EnhancedPositionsTable />);
    const buttons = screen.getAllByRole('button').filter(btn => btn.textContent?.includes('All'));
    if (buttons.length > 0) {
