      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-4 bg-card">
          <div className="space-y-2">
            ...
            <p className="text-2xl font-bold font-mono">
              ${metrics.totalEquity.toLocaleString(undefined, { maximumFractionDigits: 2 })}
            </p>
            <p className="text-xs text-muted-foreground">
              {metrics.totalBalance > 0 && (
                <>Balance: ${metrics.totalBalance.toLocaleString(undefined, { maximumFractionDigits: 2 })}</>
              )}
            </p>
          </div>
        </Card>
        ...
        <Card className={`p-4 ${metrics.marginLevel > 80 ? 'border-yellow-500/30 bg-background/5' : 'bg-card'}`}>
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Used: ${metrics.totalUsedMargin.toLocaleString(undefined, { maximumFractionDigits: 2 })}
            </p>
          </div>
        </Card>
        ...
        <Card className="p-4 bg-card">
          <div className="space-y-2">
            ...
            <p className="text-2xl font-bold font-mono">
              ${metrics.totalAvailableMargin.toLocaleString(undefined, { maximumFractionDigits: 2 })}
            </p>
            <p className="text-xs text-muted-foreground">
              {metrics.totalBalance > 0 && (
                (metrics.totalAvailableMargin / metrics.totalBalance * 100).toFixed(1) + '% available'
              )}
            </p>
          </div>
        </Card>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className={`font-semibold ${metrics.unrealizedPnL >= 0 ? 'text-profit' : 'text-loss'}`}>
                {metrics.unrealizedPnL >= 0 ? '+' : ''}${metrics.unrealizedPnL.toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Realized P&L</span>
              <span className={`font-semibold ${metrics.realizedPnL >= 0 ? 'text-profit' : 'text-loss'}`}>
        ...
        <Card className="p-4 bg-card">
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`font-semibold ${performanceMetrics.avgReturn >= 0 ? 'text-profit' : 'text-loss'}`}>
            ...
            <div className="border-t border-border pt-4 space-y-2">
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-loss font-semibold">{performanceMetrics.worstTrade.toFixed(2)}%</span>
              </div>
            </div>
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="p-4 bg-card">
          ...
          {assetAllocation.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground text-sm py-4">No positions</p>
          )}
        </Card>
      </div>
    </div>
