...
export const MultiStepProgress: React.FC<MultiStepProgressProps> = ({
  ...
  const sizeClasses = {
    ...
    lg: { container: 'h-16', circle: 'w-10 h-10', line: 'h-3', text: 'text-base' },
  };
        ...
        <div key={step.id} className="flex items-center flex-1">
          <div className="flex flex-col items-center space-y-2 flex-1">
            ...
            >
              {step.status === 'completed' ? (
                <svg className={cn("text-white", size === 'sm' ? 'w-3 h-3' : size === 'md' ? 'w-4 h-4' : 'w-5 h-5')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <span className={cn("text-center", sizeClasses[size].text, {
          ...
          {index < steps.length - 1 && (
            <div
              ...
              style={{
                animationDuration: step.status === 'completed' ? '0.8s' : '0.5s',
              }}
            />
          )}
        </div>
