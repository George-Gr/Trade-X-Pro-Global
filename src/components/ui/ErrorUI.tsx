  ...
  return (
    <div className="flex items-start gap-3 p-3 bg-red-50 border border-red-200 rounded-md">
      ...
      <div className="flex-1 min-w-0">
        ...
        {description && (
          <div className="text-xs text-red-700 mt-1">{description}</div>
        )}
        {error && (
          <div className="text-xs text-red-600 mt-1 font-mono break-words">
            {error}
          </div>
        )}
      </div>
    ...
    <div className="flex items-center gap-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md mb-4">
      ...
      <div className="flex-1">
        ...
        <div className="text-xs text-yellow-700 mt-0.5">
          Unable to connect to the server. Real-time updates are disabled. Your
          data will sync when connection restores.
        </div>
      </div>
      {onRetry && (
        <Button
 ...
 */
export const RealtimeErrorAlert: React.FC<{ onRetry?: () => void }> = ({
  onRetry,
}) => {
  return (
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-yellow-700 mt-1">
              Live data updates are temporarily unavailable. Data will be
              refreshed automatically when the connection recovers.
            </div>
            {onRetry && (
              <Button
                size="sm"
