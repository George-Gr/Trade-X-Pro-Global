...
import { SidebarErrorBoundary, NavigationItemErrorBoundary } from '@/components/ui/SidebarErrorBoundary';
...
describe('SidebarErrorBoundary', () => {
  ...
  it('should catch and handle errors gracefully', () => {
    ...
    expect(screen.getByText('We\'re experiencing issues with the navigation menu. This could be due to a temporary error or network issue.')).toBeInTheDocument();
    expect(screen.getByText('Try Again')).toBeInTheDocument();
    expect(screen.getByText('Reload Page')).toBeInTheDocument();

    // Should not show the normal component content
    expect(screen.queryByText('Test Component')).not.toBeInTheDocument();
    expect(screen.queryByText('This should render normally')).not.toBeInTheDocument();
  });
  ...
  it('should display error details when clicked', () => {
    ...
    expect(screen.getByText('Error Details (for developers)')).toBeInTheDocument();
    expect(screen.getByText('Test error from throwing component')).toBeInTheDocument();
  });
  ...
  it('should call custom onError handler when error occurs', () => {
    ...
    expect(mockOnError).toHaveBeenCalledWith(
      expect.objectContaining({ message: 'Test error from throwing component' }),
  ...
  it('should work with custom fallback component', () => {
    ...
    expect(screen.getByText('Custom Error: Test error from throwing component')).toBeInTheDocument();
...
describe('NavigationItemErrorBoundary', () => {
  ...
  it('should show error state for navigation item errors', () => {
    ...
    expect(screen.getByText('Dashboard').parentElement).toHaveAttribute('title', expect.stringContaining('is temporarily unavailable due to an error'));
