  ...
  describe("Compliance Tests - Limits & Regulations", () => {
    ...
    it("should flag suspicious withdrawal patterns", () => {
      ...
      const flagSuspicious = (withdrawals: Withdrawal[]) => {
        const timeDiffs = withdrawals
          .slice(1)
          .map((w, i) => w.timestamp - withdrawals[i].timestamp);

        // Flag if multiple withdrawals within 5 minutes
        const rapidWithdrawals =
          timeDiffs.filter((diff) => diff < 300000).length >= 3;
    ...
    it("should maintain 7-year retention for withdrawal records", () => {
      ...
      retentionExpiry.setFullYear(
        retentionExpiry.getFullYear() + retentionYears,
      );

      const withdrawal = {
        created_at: createdDate,
        retention_until: retentionExpiry,
      };
