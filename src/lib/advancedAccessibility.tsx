...
export function useColorContrast() {
  ...
  const checkElementContrast = useCallback((element: HTMLElement): number => {
...
export function useAriaLabels() {
  ...
  const [labelStats, setLabelStats] = useState<{ total: number; labeled: number; unlabeled: number }>({
    total: 0,
    labeled: 0,
    unlabeled: 0
  });
...
export function useAccessibilityTesting() {
  ...
  const runFullAudit = useCallback(async () => {
    ...
    const auditResults = {
      ...
      colorContrast: Object.values(contrastResults).every(ratio => ratio >= 4.5),
    ...
    return {
      ...auditResults,
      overallScore: Object.values(auditResults).filter(Boolean).length / Object.keys(auditResults).length * 100
    };
