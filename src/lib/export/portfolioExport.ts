...
export const exportToCSV = (data: PortfolioExportData): string => {
  ...
  lines.push(
    `Balance,$${data.balance.toLocaleString("en-US", { maximumFractionDigits: 2 })}`,
  );
  lines.push(
    `Total Equity,$${data.equity.toLocaleString("en-US", { maximumFractionDigits: 2 })}`,
  );
  lines.push(`Margin Level,${data.marginLevel.toFixed(2)}%`);
  lines.push(
    `Total P&L,$${data.totalPnL.toLocaleString("en-US", { maximumFractionDigits: 2 })}`,
  );
