...
export class AnalyticsManager {
  ...
  private setupEventListeners() {
    ...
    this.addEventListenerWithCleanup(document, "submit", (e) => {
    ...
    // Exit intent
    this.addEventListenerWithCleanup(document, "mouseleave", (e) => {
      if (e.clientY <= 0) {
  ...
  private checkFunnelProgression(event: UserEvent) {
    this.funnels.forEach((funnel, funnelId) => {
      const currentStep = funnel.steps.find(
        (step) => !this.isStepCompleted(funnelId, step.id),
      );

      if (currentStep && this.matchesEvent(currentStep, event)) {
        this.completeFunnelStep(funnelId, currentStep.id);
      }
    });
  }
  ...
  private isStepCompleted(funnelId: string, stepId: string): boolean {
  ...
  private completeFunnelStep(funnelId: string, stepId: string) {
  ...
  private updateFunnelMetrics(funnelId: string) {
    ...
    const completedSteps = funnel.steps.filter(
      (step) => step.actualCount > 0,
    ).length;

    funnel.conversionRate =
      funnel.steps[0].actualCount > 0
        ? (funnel.steps[totalSteps - 1].actualCount /
            funnel.steps[0].actualCount) *
          100
        : 0;
  ...
  private analyzeSession(session: SessionData) {
    ...
    const avgTimePerPage = session.duration / session.pageViews;
    ...
    this.sendSessionToAnalytics(session);
  }
  ...
  private sendSessionToAnalytics(session: SessionData) {
