...
export function getActionableErrorMessage(
  ...
  if (errorLower.includes('insufficient balance') || errorLower.includes('insufficient funds')) {
    return {
      ...
      suggestion: "Deposit additional funds or reduce your position size to proceed."
    };
  }

  if (errorLower.includes('invalid quantity') || errorLower.includes('quantity')) {
    ...
    if (errorLower.includes('max_quantity') || errorLower.includes('maximum')) {
      return {
        ...
        description: "The requested quantity exceeds the maximum allowed for this asset.",
        suggestion: "Reduce your order quantity to stay within the allowed limits."
      };
    }
    return {
      title: "Order Failed: Invalid Quantity",
      description: "The quantity specified is not valid for this asset.",
      suggestion: "Check the asset specifications and ensure your quantity is within allowed limits."
    };
  }
  ...
  if (errorLower.includes('invalid symbol') || errorLower.includes('symbol')) {
    return {
      ...
      description: "The trading symbol is not recognized or not available for trading.",
      suggestion: "Verify the symbol is correct and that the asset is currently tradable."
    };
  }
  ...
  if (errorLower.includes('market closed') || errorLower.includes('trading hours')) {
    return {
      ...
      description: "This asset is not available for trading during current market hours.",
      suggestion: "Try again during regular trading hours for this asset."
    };
  }

  if (errorLower.includes('leverage exceeds') || errorLower.includes('leverage')) {
    return {
      title: "Order Failed: Leverage Limit Exceeded",
      description: "The leverage for this asset exceeds your account's maximum allowed leverage.",
      suggestion: "Choose an asset with lower leverage or upgrade your account to access higher leverage."
    };
  }

  if (errorLower.includes('account suspended') || errorLower.includes('account closed')) {
    return {
      title: "Order Failed: Account Restricted",
      description: "Your trading account is currently suspended or closed.",
      suggestion: "Contact support to resolve account restrictions and restore trading access."
    };
  }

  if (errorLower.includes('kyc') || errorLower.includes('verification required')) {
    return {
      title: "Order Failed: Identity Verification Required",
      description: "You need to complete KYC verification before trading.",
      suggestion: "Complete the KYC verification process in your account settings."
    };
  }
  ...
  if (errorLower.includes('network error') || errorLower.includes('connection')) {
    return {
  ...
  if (errorLower.includes('timeout') || errorLower.includes('request timeout')) {
    return {
      ...
      suggestion: "Try again in a moment. High market volatility can cause temporary delays."
    };
  }

  // Authentication Errors
  if (errorLower.includes('unauthorized') || errorLower.includes('authentication') || errorLower.includes('login')) {
    return {
  ...
  if (context === 'order_submission') {
    return {
      title: "Order Failed",
      description: errorStr || "An unexpected error occurred while submitting your order.",
  ...
  if (context === 'form_validation') {
    return {
      ...
      suggestion: "Review the form fields for any missing or incorrect information."
    };
  }
  ...
  if (context === 'data_fetching') {
    return {
      ...
      suggestion: "Check your connection and refresh the page, or try again in a moment."
    };
  }
...
export function formatFieldError(
  ...
  if (fieldName.toLowerCase().includes('volume') || fieldName.toLowerCase().includes('quantity')) {
