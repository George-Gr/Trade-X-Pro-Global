import React, { Suspense, lazy, ComponentType, ComponentProps } from 'react';import { logger } from '@/lib/logger';
  ...
  return function LazyWrapper(props: ComponentProps<T>) {
    ...
    return (
      <ErrorBoundary
        ...
        fallback={
          fallback ? React.createElement(fallback) : <ComponentLoadingSkeleton />
        }
      >
        <Suspense
          fallback={fallback ? React.createElement(fallback) : <ComponentLoadingSkeleton />}
        >
          <LazyComponent {...props} />
        </Suspense>
      </ErrorBoundary>
...
export function useMemoryMonitor() {
  const [memory, setMemory] = React.useState<any | null>(null);
  ...
  const onRenderCallback: React.ProfilerOnRenderCallback = React.useCallback(
    (...params: Parameters<React.ProfilerOnRenderCallback>) => {
      ...
      if (actualDuration > 16) {
        logger.warn(`Slow render detected: ${params[0]} took ${actualDuration.toFixed(2)}ms`, {
          component: params[0]
        });
      }
    },
