 ...
 */
export const formatTooltipValue = (value: unknown, fractionDigits: number = 2): string => {
  ...
  if (typeof value === 'string') {
    ...
    if (!Number.isNaN(n)) return `${n.toLocaleString(undefined, { maximumFractionDigits: fractionDigits })}`;
    return value;
  }
