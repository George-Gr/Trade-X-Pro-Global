...
export function useColorContrastVerification() {
  ...
  const getContrastLevel = useCallback((ratio: number, fontSize: 'normal' | 'large' = 'normal'): 'fail' | 'aa' | 'aaa' => {
    if (fontSize === 'large') {
  ...
  const checkElementContrast = useCallback((element: HTMLElement): ContrastResult | null => {
    ...
    const wcag = getContrastLevel(ratio, element.tagName === 'H1' || element.tagName === 'H2' ? 'large' : 'normal');
  ...
  const checkPageContrast = useCallback(() => {
    ...
    const aaCompliance = (results.filter(r => r.wcag === 'aa' || r.wcag === 'aaa').length / Math.max(total, 1)) * 100;
    const aaaCompliance = (results.filter(r => r.wcag === 'aaa').length / Math.max(total, 1)) * 100;
  ...
  const getElementsByContrastLevel = useCallback((level: 'fail' | 'aa' | 'aaa') => {
    return contrastResults.filter(result => result.wcag === level);
  }, [contrastResults]);

  // Highlight failing elements
  const highlightFailingElements = useCallback((shouldHighlight: boolean) => {
...
export function useVisualAccessibilityPreferences() {
  ...
  const updatePreference = useCallback(<K extends keyof typeof preferences>(key: K, value: typeof preferences[K]) => {
...
export function useAccessibilityTesting() {
  ...
  useEffect(() => {
    ...
    // Find live regions
    const liveRegionElements = Array.from(document.querySelectorAll('[aria-live], [role="status"], [role="alert"]'));
    ...
    const newTests = [
      {
        ...
        message: hierarchyValid ? 'Heading hierarchy is valid' : 'Heading hierarchy has issues'
      },
      {
        id: 'live-regions',
  ...
  const runFullAudit = useCallback(() => {
    ...
    const totalHeadings = Object.values(headingStats).reduce((sum, count) => sum + count, 0);
