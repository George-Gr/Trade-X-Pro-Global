...
describe('Navigation Configuration', () => {
  ...
  describe('Navigation Item Retrieval', () => {
    ...
    it('should sort items by order', () => {
      const mainItems = getNavigationItemsBySection('main');
      const sorted = [...mainItems].sort((a, b) => (a.order || 999) - (b.order || 999));
  ...
  describe('Permission-Based Filtering', () => {
    it('should filter items for user role', () => {
      ...
      const filteredItems = filterNavigationItemsByRoles(NAVIGATION_CONFIG.main, userRoles);
      
      expect(filteredItems.length).toBeGreaterThan(0);
      filteredItems.forEach(item => {
        expect(item.requiredRoles).toBeDefined();
        expect(item.requiredRoles?.some(role => userRoles.includes(role))).toBe(true);
      });
    });

    it('should filter items for admin role', () => {
      const adminRoles = ['admin'];
      const filteredItems = filterNavigationItemsByRoles(NAVIGATION_CONFIG.main, adminRoles);
    ...
    it('should show no items when no roles specified (authenticated nav)', () => {
      const filteredItems = filterNavigationItemsByRoles(NAVIGATION_CONFIG.main, []);
  ...
  describe('Backward Compatibility', () => {
    it('should provide backward compatible nav items', () => {
      const legacyItems = getAllNavigationItems().filter(item => 
        item.id !== 'logout' && item.id !== 'profile'
      );
