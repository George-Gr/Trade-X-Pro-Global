...
describe('Margin Call Detection - Notifications', () => {
  ...
  it('should include metadata in notification', () => {
    ...
    expect(
      (notification.metadata as unknown as Record<string, unknown>).marginLevel
    ).toBe(100);
    expect(
      (notification.metadata as unknown as Record<string, unknown>).severity
    ).toBe(MarginCallSeverity.URGENT);
  });
});
...
describe('Margin Call Detection - Recommended Actions', () => {
  it('should recommend urgent actions for critical margin (< 50%)', () => {
    ...
    expect(
      actions.some((a) => a.action.toLowerCase().includes('deposit'))
    ).toBe(true);
  });
  ...
  it('should recommend priority actions for urgent margin (50-100%)', () => {
    ...
    expect(actions.some((a) => a.action.toLowerCase().includes('close'))).toBe(
      true
    );
  });
  ...
  it('should recommend cautionary actions for standard margin (100-150%)', () => {
    ...
    expect(actions.length).toBeGreaterThan(0);
    expect(
      actions.some((a) => a.action.toLowerCase().includes('monitor'))
    ).toBe(true);
  });
...
describe('Margin Call Detection - Risk Metrics', () => {
  ...
  it('should calculate concentration risk for large positions', () => {
    ...
    expect(metricsHigh.concentrationRisk).toBeGreaterThan(
      metricsLow.concentrationRisk
    );
  });
...
describe('Margin Call Detection - Validation and Formatting', () => {
  ...
  it('should format margin call status for display', () => {
    expect(formatMarginCallStatus(MarginCallStatus.NOTIFIED)).toBe(
      'Margin Call Active'
    );
    expect(formatMarginCallStatus(MarginCallStatus.RESOLVED)).toBe(
      'Margin Call Resolved'
    );
    expect(formatMarginCallStatus(MarginCallStatus.ESCALATED)).toBe(
      'Escalated to Liquidation'
    );
  });
