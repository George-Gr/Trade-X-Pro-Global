 ...
 */
export const ASSET_SLIPPAGE_CONFIG: Record<string, {
...
export type SlippageCalculationInput = z.infer<typeof SlippageCalculationSchema>;
 ...
 */
export function calculateSlippage(input: SlippageCalculationInput): SlippageResult {
  // Validate input
  ...
  const { symbol, marketPrice, orderQuantity, side, conditions, seed } = validation.data;
  ...
  // Calculate after-hours penalty
  const afterHoursPenalty = calculateAfterHoursPenalty(config, conditions.isAfterHours);

  // Calculate base slippage
  const baseSlippage = calculateBaseSlippage(config, random);

  // Calculate total slippage
  const totalSlippage = baseSlippage * volatilityMultiplier * sizeMultiplier * afterHoursPenalty;
