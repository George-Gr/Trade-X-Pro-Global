...
export function calculatePnLPercentage({
  ...
  current_price,
  leverage,
}: Pick<Position, 'entry_price' | 'current_price' | 'leverage'>): number {
  return +(
...
export function getPositionColor({
  ...
  entry_price,
  current_price,
}: Pick<Position, 'side' | 'entry_price' | 'current_price'>): string {
  const pnl =
    side === 'long' ? current_price - entry_price : entry_price - current_price;
