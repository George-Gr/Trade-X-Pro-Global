...
describe('Focus Ring Enhancement CSS Tests', () => {
  ...
  it('should have 3px focus rings for input elements', () => {
    ...
    input.style.boxShadow = '0 0 0 6px hsl(var(--focus-ring-color) / 0.3), 0 0 0 8px hsl(var(--focus-ring-color) / 0.2)';

    // Verify styles are applied correctly
    expect(input.style.outline).toContain('3px solid');
    expect(input.style.outlineOffset).toBe('2px');
    expect(input.style.boxShadow).toContain('6px hsl(var(--focus-ring-color) / 0.3)');
  ...
  it('should have enhanced focus rings for button elements', () => {
    ...
    button.style.boxShadow = '0 0 0 6px hsl(var(--focus-ring-color) / 0.3), 0 0 0 8px hsl(var(--focus-ring-color) / 0.2)';

    // Verify styles are applied correctly
    expect(button.style.outline).toContain('3px solid');
    expect(button.style.outlineOffset).toBe('2px');
    expect(button.style.boxShadow).toContain('6px hsl(var(--focus-ring-color) / 0.3)');
  ...
  it('should have high contrast focus rings', () => {
    ...
    element.style.boxShadow = '0 0 0 8px hsl(var(--primary-contrast-light) / 0.8), 0 0 0 10px hsl(var(--primary-contrast-light) / 0.6)';
    ...
    expect(element.style.boxShadow).toContain('8px hsl(var(--primary-contrast-light) / 0.8)');
  ...
  it('should support animated focus rings', () => {
    ...
    expect(element.style.outlineOffset).toBe('2px');
    expect(element.style.animation).toContain('focus-pulse 2s ease-in-out infinite');
    ...
    for (let i = 0; i < (keyframes?.length || 0); i++) {
      if (keyframes && keyframes[i] && keyframes[i].cssText?.includes('focus-pulse')) {
        hasFocusPulse = true;
        break;
      }
    }
  ...
  it('should have dark mode focus rings', () => {
    ...
    element.style.outline = '3px solid hsl(var(--focus-color))';
    element.style.outlineOffset = '2px';
    element.style.boxShadow = '0 0 0 6px hsl(var(--focus-ring-color) / 0.4), 0 0 0 8px hsl(var(--focus-ring-color) / 0.3)';

    // Verify dark mode styles are applied correctly
    expect(element.style.outline).toContain('3px solid');
    expect(element.style.outlineOffset).toBe('2px');
    expect(element.style.boxShadow).toContain('6px hsl(var(--focus-ring-color) / 0.4)');
  ...
  it('should support role="button" focus rings', () => {
    ...
    element.style.outline = '3px solid hsl(var(--focus-color))';
    element.style.outlineOffset = '2px';
    element.style.boxShadow = '0 0 0 6px hsl(var(--focus-ring-color) / 0.3), 0 0 0 8px hsl(var(--focus-ring-color) / 0.2)';
  ...
  it('should have proper outline-offset for focus rings', () => {
    ...
    // Clean up
    [input, button, link].forEach(element => document.body.removeChild(element));
  });
});
