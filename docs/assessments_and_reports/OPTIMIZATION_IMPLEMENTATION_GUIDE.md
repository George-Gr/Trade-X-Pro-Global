...
```tsx
<TradeXOptimizationSuite
    customConfig={OptimizationPresets.balanced}
/>
```
...
```tsx
<TradeXOptimizationSuite
    customConfig={OptimizationPresets.minimal}
/>
```
...
function MyComponent() {
    const { trackInteraction, trackRender } = usePerformanceTracking('MyComponent');
