...
find src/ -name "*.tsx" -exec sed -i.bak 's/import React, {/import {/g' {} \;
find src/ -name "*.tsx" -exec sed -i.bak 's/} from "react";//g' {} \;
find src/ -name "*.tsx" -exec sed -i.bak 's/import React from "react";//g' {} \;
...
| tail -1 || echo 5173)
curl -f "http://localhost:${PORT}/" > /dev/null && echo "✅ App accessible on port $PORT" || echo "❌ App failed to start"

# Kill dev server by PID only
