...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [ordersCount, fillsCount, ledgerCount, positionsCount] = await Promise.all([
      supabase.from('orders_archive').select('id', { count: 'exact', head: true }),
      supabase.from('fills_archive').select('id', { count: 'exact', head: true }),
      supabase.from('ledger_archive').select('id', { count: 'exact', head: true }),
      supabase.from('positions_archive').select('id', { count: 'exact', head: true }),
    ]);
