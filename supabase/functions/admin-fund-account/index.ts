...
const FundAccountSchema = z.object({
  ...
  amount: z
    ...
    .max(100000, "Maximum funding amount is $100,000 per operation"),
  description: z
    .string()
  ...
  try {
    ...
    if (!rateLimitOk) {
      ...
      return new Response(
        JSON.stringify({
          error:
            "Too many requests. Please wait before performing another funding operation.",
        }),
        {
          status: 429,
          headers: {
    ...
    // Create ledger entry
    const { error: ledgerError } = await supabaseClient.from("ledger").insert({
      user_id,
