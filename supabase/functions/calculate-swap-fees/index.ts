...
serve(async (req) => {
  ...
  try {
    ...
    if (!positions || positions.length === 0) {
      ...
      return new Response(
        JSON.stringify({ message: 'No positions to charge swap fees', total: 0 }),
    ...
    for (const position of positions) {
      try {
        ...
        const positionValue = position.quantity * 100000 * (position.current_price || position.entry_price);
        ...
        if (profileError) {
          console.error(`Error fetching profile for user ${position.user_id}:`, profileError);
          continue;
        }
        ...
        if (updateError) {
          console.error(`Error updating balance for user ${position.user_id}:`, updateError);
          continue;
        }
        ...
        console.log(`Charged ${swapFee.toFixed(2)} swap fee for position ${position.id}`);
      } catch (error) {
        console.error(`Error processing swap for position ${position.id}:`, error);
        swapResults.push({
          position_id: position.id,
    ...
    console.log(`Swap fees calculation complete. Total fees: ${totalSwapFees.toFixed(2)}`);
