...
serve(async (req) => {
  ...
  try {
    ...
    if (!CRON_SECRET || providedSecret !== CRON_SECRET) {
      console.error(
        'Unauthorized access attempt to execute-stop-loss-take-profit'
      );
    ...
    for (const position of positions) {
      try {
        ...
        if (priceError || !priceData) {
          console.error(
            `Failed to get price for ${position.symbol}:`,
            priceError
          );
          continue;
        }
        ...
        if (shouldClose) {
          console.log(
            `Triggering ${triggerType} for position ${position.id} at price ${currentPrice}`
          );
          ...
          if (closeError) {
            ...
            console.log(
              `Successfully closed position ${position.id} via ${triggerType}`
            );
            triggeredCount++;
    ...
    console.log(
      `SL/TP check complete. Triggered ${triggeredCount} positions out of ${positions.length}`
    );
