...
const FOREX_MOCK_DATA: Record<string, { base: number, volatility: number }> = {
  ...
  'OANDA:XAU_USD': { base: 2350.00, volatility: 15.00 },
  'OANDA:XAG_USD': { base: 28.50, volatility: 0.30 },
  'OANDA:BCO_USD': { base: 82.50, volatility: 1.20 },
  'OANDA:WTICO_USD': { base: 78.00, volatility: 1.10 },
  'OANDA:NATGAS_USD': { base: 2.85, volatility: 0.08 },
  'OANDA:BTC_USD': { base: 67500.00, volatility: 800.00 },
  'OANDA:ETH_USD': { base: 3450.00, volatility: 60.00 },
};

async function generateForexPrice(symbol: string): Promise<SignedPriceResponse | null> {
...
async function generateStockFallbackPrice(symbol: string): Promise<SignedPriceResponse | null> {
...
const stockCache = new Map<string, { data: SignedPriceResponse; timestamp: number }>();
  ...
  try {
    ...
    if (cached && (now - cached.timestamp) < STOCK_CACHE_TTL_MS) {
      ...
      return new Response(
        ...
        { headers: { ...rateLimitHeaders, 'Content-Type': 'application/json', 'X-Cache': 'hit' } }
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          if (cached) {
            ...
            return new Response(
              JSON.stringify(cached.data),
              { headers: { ...rateLimitHeaders, 'Content-Type': 'application/json', 'X-Cache': 'stale' } }
            );
          }
          ...
          if (fallback) {
            ...
            return new Response(
              ...
              { headers: { ...rateLimitHeaders, 'Content-Type': 'application/json', 'X-Cache': 'fallback' } }
            );
          }
          return new Response(
            JSON.stringify({ c: 0, d: 0, dp: 0, h: 0, l: 0, o: 0, pc: 0, t: Math.floor(Date.now()/1000) }),
            { headers: { ...rateLimitHeaders, 'Content-Type': 'application/json', 'X-Cache': 'empty' } }
          );
        }
        return new Response(
      ...
      return new Response(
        ...
        { headers: { ...rateLimitHeaders, 'Content-Type': 'application/json', 'X-Cache': 'miss' } }
      );
    } catch (e) {
