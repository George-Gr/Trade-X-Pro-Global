...
const ClosePositionSchema = z.object({
  ...
  quantity: z
    ...
    .optional(),
  idempotency_key: z.string().min(1, "Idempotency key required").optional(),
  ...
  try {
    ...
    if (!rateLimitOk) {
      ...
      return new Response(
        JSON.stringify({
          error:
            "Too many requests. Please wait before closing another position.",
        }),
        {
          status: 429,
          headers: {
      ...
      return new Response(
        JSON.stringify({
          ...
          details: validation.error.issues
            .map(
              (issue: unknown) =>
                `${(issue as { path: string[]; message: string }).path.join(".")}: ${(issue as { path: string[]; message: string }).message}`,
            )
            .join(", "),
        }),
        {
    ...
    if (!currentPrice || currentPrice <= 0) {
      return new Response(JSON.stringify({ error: "Invalid market price" }), {
        status: 503,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const reason: ClosureReason =
      ((validation.data as unknown as Record<string, unknown>)
        .reason as ClosureReason) || ClosureReason.MANUAL_USER;

    if (
      reason === ClosureReason.LIQUIDATION ||
      reason === ClosureReason.MARGIN_CALL
    ) {
    ...
    console.log("Exit price calculated:", {
      currentPrice,
      exitPrice,
      slippageAmount,
    });
    ...
    const closureId =
      closeResult && Array.isArray(closeResult) && closeResult[0]?.closure_id
    ...
    return new Response(
      JSON.stringify({
        data: {
          ...
          quantity_remaining: isPartialClose
