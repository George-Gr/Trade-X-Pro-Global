  ...
  try {
    ...
    for (const uid of usersToCheck) {
      try {
        ...
        // Calculate metrics
        const totalExposure = (positions || []).reduce((sum: number, pos: Position) => {
          return sum + Math.abs(pos.quantity * pos.current_price);
        }, 0);

        const totalMarginUsed = (positions || []).reduce((sum: number, pos: Position) => {
          return sum + (pos.margin_used || 0);
        }, 0);

        const equity = (profile?.balance || 0) + 
        ...
        const marginLevel = totalMarginUsed > 0 
          ...
          if (riskStatus !== 'safe') {
            const { error: alertError } = await supabase
              ...
              .insert({
                ...
                exceedance_percentage: Math.max(0, (150 - marginLevel) / 150 * 100),
        ...
        await supabase
          ...
          .insert({
            ...
            max_concentration: totalExposure > 0 
              ? Math.max(...(positions || []).map((p: Position) => 
                  Math.abs(p.quantity * p.current_price) / totalExposure * 100
                ), 0)
              : 0,
            portfolio_correlation: 50, // Simplified
            var_estimate: (totalExposure * 0.15 * 1.645) / Math.max(equity, 1) * 100,
