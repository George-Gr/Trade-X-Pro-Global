...
function assessMarginStatus(marginLevel: number, timeInCriticalMinutes: number = 0): MarginStatus {
  if (marginLevel >= 200) {
    return {
      level: marginLevel,
      severity: 'SAFE',
  ...
  try {
    ...
    for (const user of users || []) {
      ...
      const timeInCritical = existingCall 
        ? Math.floor((Date.now() - new Date(existingCall.triggered_at).getTime()) / (1000 * 60))
        : 0;
      ...
      if (status.shouldNotify && !existingCall) {
        // New margin call
        const gracePeriodExpires = new Date(Date.now() + GRACE_PERIOD_MINUTES * 60 * 1000);
        ...
        const { data: newCall } = await supabase
          ...
          .insert({
            ...
            grace_period_expires_at: status.severity === 'CRITICAL' ? gracePeriodExpires : null,
          })
          .select()
          .single();
        ...
        const notificationTitle = status.severity === 'WARNING' 
          ? '⚠️ Margin Warning'
          : status.severity === 'CRITICAL'
          ? '🚨 CRITICAL Margin Call'
          : '⛔ Liquidation Imminent';

        const notificationMessage = status.severity === 'WARNING'
        ...
        await supabase.functions.invoke('send-notification', {
          body: {
            ...
            data: {
              margin_level: marginLevel,
              severity: status.severity,
              grace_period_minutes: status.severity === 'CRITICAL' ? GRACE_PERIOD_MINUTES : 0,
            },
            send_email: true,
          },
        });
        ...
        await supabase.functions.invoke('send-notification', {
          body: {
            ...
            title: shouldLiquidate ? '⛔ Liquidation Starting' : '🚨 Margin Call Escalated',
    ...
    console.log(`Margin check complete: ${newMarginCalls} new calls, ${escalations} escalations, ${liquidationsTriggered} liquidations`);
