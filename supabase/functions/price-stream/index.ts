  ...
  for (const provider of providers) {
    try {
      ...
      if (data && data.c > 0) {
        ...
        console.error(`${provider.name} failed for ${symbol}:`, error instanceof Error ? error.message : String(error));
        continue;
      }
  }
          ...
          intervalId = setInterval(async () => {
            ...
            for (const symbol of subscribedSymbols) {
              try {
                ...
                prices[symbol] = { error: error instanceof Error ? error.message : 'Unknown error' };
              }
            }
