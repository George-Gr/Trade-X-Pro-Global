-- Add defense-in-depth validation to execute_order_atomic function
-- This adds explicit KYC and account status checks inside the SECURITY DEFINER function
...
CREATE OR REPLACE FUNCTION public.execute_order_atomic(
    ...
    p_slippage numeric DEFAULT 0.0005
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    -- DEFENSE-IN-DEPTH: Validate user authorization (KYC + account status)
    -- This provides an additional security layer beyond edge function checks
    ...
    -- Validate KYC status
    IF v_user_profile.kyc_status != 'approved' THEN
        RAISE EXCEPTION 'KYC verification required. Current status: %', v_user_profile.kyc_status;
    END IF;

    -- Validate account status
    IF v_user_profile.account_status != 'active' THEN
        RAISE EXCEPTION 'Account is not active. Current status: %', v_user_profile.account_status;
    END IF;

    -- Use validated profile data
    v_user_balance := v_user_profile.balance;
    v_user_equity := v_user_profile.equity;
    v_user_margin_used := v_user_profile.margin_used;
...
-- Add defense-in-depth validation to close_position_atomic function
CREATE OR REPLACE FUNCTION public.close_position_atomic(
    p_user_id uuid,
    p_position_id uuid,
    p_close_quantity numeric,
    p_current_price numeric,
    p_idempotency_key text,
    p_slippage numeric DEFAULT 0.0005
)
...
BEGIN
    -- DEFENSE-IN-DEPTH: Validate user authorization (KYC + account status)
    SELECT kyc_status, account_status, balance
    INTO v_user_profile
    FROM public.profiles
    WHERE id = p_user_id
    FOR UPDATE;
